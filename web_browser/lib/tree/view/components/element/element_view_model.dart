...
class ElementViewModel extends _$ElementViewModel {
  /// 要素の幅
  double get elementWidth => ref.watch(treeSettingsProvider).elementWidth;

  /// 要素のパディング
  double get elementPadding =>
      ref.watch(treeSettingsProvider).elementHorizontalSpacing;
  @override
  ElementState build(NodePath nodePath) {
    String title = ref.watch(browserNodeFromPathProvider(nodePath)).title;
    (double, double) position = _calcElementPos();
